...
print(moderated)
X_train, X_test, y_train, y_test = split_acti1.main(pca_dims, rate, test_data_ratio=0.3)
...
seed(2020)
model = Sequential()
model.add(Conv1D(100, 30, input_shape=(30, 1), activation='relu'))
...
new_dir = 'Acti-tracker Data Set/' + moderated + 'Hz/weights/'
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
model.save('Acti-tracker Data Set/' + moderated + 'Hz/' + 'pca=' + str(pca_dims) + '.hdf5')
